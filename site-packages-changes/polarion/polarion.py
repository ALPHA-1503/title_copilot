...
import requests
from requests import Session  # lwi
from zeep import Client, CachingClient
from zeep.plugins import HistoryPlugin
from zeep.transports import Transport  # lwi
...
class Polarion(object):
    ...
    def get_client(self, service, plugins=None):
        #lwi start
        if type(self.verify_certificate) == str and self.verify_certificate:
            _session = Session()
            _session.verify = self.verify_certificate
            _transport = Transport(session=_session)
        #lwi end
            if self.cache:
                client = CachingClient(self.services[service]['url'] + '?wsdl', plugins=plugins, transport=_transport)
            else:
                client = Client(self.services[service]['url'] + '?wsdl', plugins=plugins, transport=_transport)
        else:
            if self.cache:
        ...
        #lwi start
        if type(self.verify_certificate) == bool:
    ...
    def getRepoProjects(self, location: str = None):
        service = self.getService('Project')
        if location is None:
            default_location = f'default:/'
        else:
            default_location = f'default:/{location}'
        default_project_group = service.getProjectGroupAtLocation(default_location)
        projects = service.getDeepContainedProjects(default_project_group.uri)
        return [Project(self, project.id) for project in projects if project.id]
